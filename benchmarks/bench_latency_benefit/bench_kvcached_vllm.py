...
async def benchmark(
    *,
    backend: str,
    deployments: list[tuple[str, str, str]],  # (model_id, model_name, api_url)
    tokenizer: PreTrainedTokenizerBase,
    input_requests: list[SampleRequest],
...
def main(args: argparse.Namespace):
    ...
    # ------------------------------------------------------------------
    # Build deployment list: each item is (model_id, model_name, api_url)
    # ------------------------------------------------------------------
    deployments: list[tuple[str, str, str]] = []

    if args.model_endpoints:
        for item in args.model_endpoints:
            if "@" not in item:
                raise ValueError(
                    f"Invalid --model-endpoints entry: '{item}'. Expected MODEL@BASE_URL."
                )
            mdl, base_url_item = item.split("@", 1)
            api_url_item = f"{base_url_item}{args.endpoint}"
            deployments.append((mdl, mdl, api_url_item))
    else:
        # Fallback to legacy single-engine behaviour.
        if args.base_url is not None:
            api_url_single = f"{args.base_url}{args.endpoint}"
            # base_url_single = f"{args.base_url}"
        else:
            api_url_single = f"http://{args.host}:{args.port}{args.endpoint}"
            # base_url_single = f"http://{args.host}:{args.port}"
        mdl = args.model
        deployments.append((mdl, mdl, api_url_single))
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the model. Ignored if --model-endpoints is provided.",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        metavar="MODEL@BASEURL",
        help=(
            "Optional: specify one or more MODEL@BASE_URL pairs to benchmark multiple "
            "vLLM engines. BASE_URL should NOT include the endpoint path; e.g., "
            "'llama@http://10.0.0.1:8000'. When provided, --host/--port are ignored and "
            "requests will be distributed round-robin across the given engines. The "
            "same MODEL may appear multiple times with different URLs."
        ),
    )
    parser.add_argument(
        "--tokenizer",
